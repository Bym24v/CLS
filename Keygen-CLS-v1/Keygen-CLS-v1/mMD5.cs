...
public class mMD5
{
    ...
    /// </summary>
    static uint[] X = new uint[16];

    /// <summary>
    /// the input bytes
    /// </summary>
    public static byte[] m_byteInput;
    ...
    public static hMD5.Digest CalculateMD5Value(byte[] _bytes)
    {
        /***********vairable declaration**************/
        byte [] bMsg;	//buffer to hold bits
        uint N;			//N is the size of msg as  word (32 bit) 
        hMD5.Digest dg = new hMD5.Digest();			//  the value to be returned
    ...
    /// </summary>
    static void TransF(ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i)
    {
        a = b + hMD5.MD5Helper.RotateLeft((a + ((b & c) | (~(b) & d)) + X[k] + T[i - 1]), s);
    }

    /// <summary>
    /// perform transformatio using g((b&d) | (c & ~d) )
    /// </summary>
    static void TransG(ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i)
    {
        a = b + hMD5.MD5Helper.RotateLeft((a + ((b & d) | (c & ~d)) + X[k] + T[i - 1]), s);
    }

    /// <summary>
    /// perform transformatio using h(b^c^d)
    /// </summary>
    static void TransH(ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i)
    {
        a = b + hMD5.MD5Helper.RotateLeft((a + (b ^ c ^ d) + X[k] + T[i - 1]), s);
    }

    /// <summary>
    /// perform transformatio using i (c^(b|~d))
    /// </summary>
    static void TransI(ref uint a, uint b, uint c, uint d, uint k, ushort s, uint i)
    {
        a = b + hMD5.MD5Helper.RotateLeft((a + (c ^ (b | ~d)) + X[k] + T[i - 1]), s);
    }
    ...
    static void PerformTransformation(ref uint A, ref uint B, ref uint C, ref uint D)
    {
    ...
    static byte[] CreatePaddedBuffer()
    {
    ...
    static void CopyBlock(byte[] bMsg, uint block)
    {
